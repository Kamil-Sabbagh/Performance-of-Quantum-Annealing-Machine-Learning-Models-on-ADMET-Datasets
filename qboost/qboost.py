...
class EnsembleClassifier:
    ...
    def score(self, X, y, average='binary'):
        ...
        y_scores = self.predict(X)  # Use predict, not predict_class, to get decision function scores
...
def qboost_lambda_sweep(X_train, y_train, X_val, y_val, lambda_vals, verbose=False, **kwargs):
    ...
    for lam in tqdm(lambda_vals):
        ...
        score_dict = qb.score(X_val, y_val)
        score = score_dict['auc_roc']
...
class QBoostOvRClassifier(BaseEstimator, ClassifierMixin):
    ...
    def score(self, X, y):
        ...
        print(f"checking the score for the testing dataset!")
